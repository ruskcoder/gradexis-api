...
async function fetchClassesData(session, link, term, progressTracker) {
	...
	const fetchNew = !term || (classesCache && classesCache.term === term) || !(term && classesCache && classesCache.viewstate && classesCache.eventvalidation);
	
	let $, viewstate, eventvalidation, year;

	if (fetchNew) {
		const message = !term ? 'Fetching classes' : 'Going to term';
		progressTracker.update(65, message);
		...
		viewstate = classesCache.viewstate;
		eventvalidation = classesCache.eventvalidation;
		year = classesCache.year;
	}
